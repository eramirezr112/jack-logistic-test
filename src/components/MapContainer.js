...
class MapContainer extends Component {
      ...
      directionsService.route(
        ...
        function (response, status) {
          if (status === curMaps.DirectionsStatus.OK) {
            const center = new curMaps.LatLng(
              centerAddress.lat,
              centerAddress.lng
            );
            const origin = new curMaps.LatLng(
              shippingFromAddress.lat,
              shippingFromAddress.lng
            );
            const destination = new curMaps.LatLng(
              shippingToAddress.lat,
              shippingToAddress.lng
            );
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevProps.geoInfo !== this.props.geoInfo) {
      ...
      console.log("GEO INFO", this.props.geoInfo);
      const centerAddress = {
      ...
      for (let i = 0; i < this.markers.length; i++) {
        ...
        const point1 = new this.maps.LatLng(
          position.lat() + 0.001,
          position.lng() + 0.001
        );
        const point2 = new this.maps.LatLng(
          position.lat() - 0.001,
          position.lng() - 0.001
        );
  ...
  render() {
    return (
      <GoogleMapReact
        ...
        defaultCenter={{ lat: 9.930603935103061, lng: -84.08986684752382 }}
        defaultZoom={16}
      ></GoogleMapReact>
    );
  }
